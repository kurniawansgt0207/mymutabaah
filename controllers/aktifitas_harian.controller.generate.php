<?php
    ...
    class aktifitas_harianControllerGenerate
    {
        protected $aktifitas_harian;
        var $modulename = "aktifitas_harian";
        var $dbh;
        var $limit = 20;
        var $user = "None";
        var $ip = "";
        ...
        var $lastID = "";
        var $toolsController;
        function __construct($aktifitas_harian, $dbh) {
            $this->modulename = strtoupper($this->modulename);
            $this->aktifitas_harian = $aktifitas_harian;
            $this->dbh = $dbh;            
        ...
        function insertData(){
            ...
            $sql = "INSERT INTO aktifitas_harian ";
            $sql .= " ( ";
	    $sql .= "`id`,";
	    $sql .= "`tgl_aktifitas`,";
	    $sql .= "`user_id`,";
	    ...
	    $sql .= "`ip_address` ";
            ...
            $sql .= " VALUES (";
	    $sql .= " null,";
	    $sql .= "'".$this->toolsController->replacecharSave($this->aktifitas_harian->getTgl_aktifitas(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->aktifitas_harian->getUser_id(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->aktifitas_harian->getUser_fullname(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->aktifitas_harian->getCreated_date(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->aktifitas_harian->getCreated_by(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->aktifitas_harian->getUpdated_date(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->aktifitas_harian->getUpdated_by(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->aktifitas_harian->getIp_address(), $this->dbh)."' ";
        ...
        function updateData(){
            $datetime = date("Y-m-d H:i:s");
            $sql = "UPDATE aktifitas_harian SET ";
	    $sql .= "`id` = '".$this->toolsController->replacecharSave($this->aktifitas_harian->getId(),$this->dbh)."',";
	    $sql .= "`tgl_aktifitas` = '".$this->toolsController->replacecharSave($this->aktifitas_harian->getTgl_aktifitas(),$this->dbh)."',";
	    $sql .= "`user_id` = '".$this->toolsController->replacecharSave($this->aktifitas_harian->getUser_id(),$this->dbh)."',";
	    $sql .= "`user_fullname` = '".$this->toolsController->replacecharSave($this->aktifitas_harian->getUser_fullname(),$this->dbh)."',";
	    $sql .= "`created_date` = '".$this->toolsController->replacecharSave($this->aktifitas_harian->getCreated_date(),$this->dbh)."',";
	    $sql .= "`created_by` = '".$this->toolsController->replacecharSave($this->aktifitas_harian->getCreated_by(),$this->dbh)."',";
	    $sql .= "`updated_date` = '".$this->toolsController->replacecharSave($this->aktifitas_harian->getUpdated_date(),$this->dbh)."',";
	    $sql .= "`updated_by` = '".$this->toolsController->replacecharSave($this->aktifitas_harian->getUpdated_by(),$this->dbh)."',";
	    $sql .= "`ip_address` = '".$this->toolsController->replacecharSave($this->aktifitas_harian->getIp_address(),$this->dbh)."' ";
            $sql .= "WHERE id = '".$this->aktifitas_harian->getId()."'";                
            $execute = $this->dbh->query($sql);
        }
        
        function deleteData($id){
            $sql = "DELETE FROM aktifitas_harian WHERE id = '".$id."'";
            $execute = $this->dbh->query($sql);
        }
        
        function showData($id){
            $sql = "SELECT * FROM aktifitas_harian WHERE id = '".$this->toolsController->replacecharFind($id,$this->dbh)."'";
        ...
        function checkData($id){
            $sql = "SELECT count(*) FROM aktifitas_harian where id = '".$id."'";
            $row = $this->dbh->query($sql)->fetch();
            return $row[0];
        }
        ...
        function showDataWhereQuery(){
            ...
            if ($bsearch) {
               ...
               $where .= "       or  tgl_aktifitas like '%".$search."%'";
               $where .= "       or  user_id like '%".$search."%'";
               $where .= "       or  user_fullname like '%".$search."%'";
        ...
        function findDataWhere($where){
            $sql = "SELECT * FROM aktifitas_harian  ".$where;
            $sql .= " ORDER BY id";
            return $sql;
        }
        function findCountDataWhere($where){
            $sql = "SELECT count(id)  FROM aktifitas_harian  ".$where;
            $sql .= " ORDER BY id";
            return $sql;
        }
        ...
        function createList($sql){
            $aktifitas_harian_List = array();
            foreach ($this->dbh->query($sql) as $row) {
                    $aktifitas_harian_ = new aktifitas_harian();
                    $this->loadData($aktifitas_harian_, $row);
                    $aktifitas_harian_List[] = $aktifitas_harian_;
            }
            return $aktifitas_harian_List;            
        }

                
        function loadData($aktifitas_harian,$row){
	    $aktifitas_harian->setId($row['id']);
	    $aktifitas_harian->setTgl_aktifitas($row['tgl_aktifitas']);
	    $aktifitas_harian->setUser_id($row['user_id']);
	    $aktifitas_harian->setUser_fullname($row['user_fullname']);
	    $aktifitas_harian->setCreated_date($row['created_date']);
	    $aktifitas_harian->setCreated_by($row['created_by']);
	    $aktifitas_harian->setUpdated_date($row['updated_date']);
	    $aktifitas_harian->setUpdated_by($row['updated_by']);
	    $aktifitas_harian->setIp_address($row['ip_address']);
        ...
        function showAll(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $aktifitas_harian_list = $this->showDataAllLimit();
                ...
                require_once './views/aktifitas_harian/aktifitas_harian_list.php';
            }else{
        ...
        function showAllJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $isexport = $this->isexport ;
                $isimport = $this->isimport;
                require_once './views/aktifitas_harian/aktifitas_harian_jquery_list.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showDetail(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $aktifitas_harian_ = $this->showData($id);
                require_once './views/aktifitas_harian/aktifitas_harian_detail.php';
            }else{
                echo "You cannot access this module";
            }
        }
        function showDetailJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $aktifitas_harian_ = $this->showData($id);
                require_once './views/aktifitas_harian/aktifitas_harian_jquery_detail.php';
            }else{
                echo  "You cannot access this module";
            }
        }
        ...
        function showForm(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $aktifitas_harian_ = $this->showData($id);
                require_once './views/aktifitas_harian/aktifitas_harian_form.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showFormJQuery(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $search = isset($_REQUEST["search"]) ? $_REQUEST["search"] : "";
                $aktifitas_harian_ = $this->showData($id);
                require_once './views/aktifitas_harian/aktifitas_harian_jquery_form.php';
            }else{
                echo "You cannot access this module";
            }
        }        
	    ...
	    $tgl_aktifitas = isset($_POST['tgl_aktifitas'])?$_POST['tgl_aktifitas'] : "";
	    ...
	    $this->aktifitas_harian->setId($id);
	    $this->aktifitas_harian->setTgl_aktifitas($tgl_aktifitas);
	    $this->aktifitas_harian->setUser_id($user_id);
	    $this->aktifitas_harian->setUser_fullname($user_fullname);
	    $this->aktifitas_harian->setCreated_date($created_date);
	    $this->aktifitas_harian->setCreated_by($created_by);
	    $this->aktifitas_harian->setUpdated_date($updated_date);
	    $this->aktifitas_harian->setUpdated_by($updated_by);
	    $this->aktifitas_harian->setIp_address($ip_address);            
            $this->saveData();
        }
        public function saveData(){
            $check = $this->checkData($this->aktifitas_harian->getId());
            if($check == 0){
        ...
        public function getAktifitas_harian() {
            return $this->aktifitas_harian;
        }
        public function setAktifitas_harian($aktifitas_harian) {
            $this->aktifitas_harian = $aktifitas_harian;
        }
