<?php
    ...
    class master_shaffControllerGenerate
    {
        protected $master_shaff;
        var $modulename = "master_shaff";
        var $dbh;
        var $limit = 20;
        var $user = "None";
        var $ip = "";
        ...
        var $lastID = "";
        var $toolsController;
        function __construct($master_shaff, $dbh) {
            $this->modulename = strtoupper($this->modulename);
            $this->master_shaff = $master_shaff;
            $this->dbh = $dbh;            
	    ...
	    $sql .= "`nama_shaff` ";
            ...
            $sql .= " VALUES (";
	    $sql .= " null,";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_shaff->getKode_shaff(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->master_shaff->getNama_shaff(), $this->dbh)."' ";
        ...
        function updateData(){
            $datetime = date("Y-m-d H:i:s");
            $sql = "UPDATE master_shaff SET ";
	    $sql .= "`id` = '".$this->toolsController->replacecharSave($this->master_shaff->getId(),$this->dbh)."',";
	    $sql .= "`kode_shaff` = '".$this->toolsController->replacecharSave($this->master_shaff->getKode_shaff(),$this->dbh)."',";
	    $sql .= "`nama_shaff` = '".$this->toolsController->replacecharSave($this->master_shaff->getNama_shaff(),$this->dbh)."' ";
            $sql .= "WHERE id = '".$this->master_shaff->getId()."'";                
            $execute = $this->dbh->query($sql);
        }
        
        function deleteData($id){
            $sql = "DELETE FROM master_shaff WHERE id = '".$id."'";
            $execute = $this->dbh->query($sql);
        }
        
        function showData($id){
            $sql = "SELECT * FROM master_shaff WHERE id = '".$this->toolsController->replacecharFind($id,$this->dbh)."'";

            $row = $this->dbh->query($sql)->fetch();
            $this->loadData($this->master_shaff, $row);
            
            return $this->master_shaff;
        }
        
        function checkData($id){
            $sql = "SELECT count(*) FROM master_shaff where id = '".$id."'";
            $row = $this->dbh->query($sql)->fetch();
            return $row[0];
        }
        ...
        function showDataWhereQuery(){
            ...
            if ($bsearch) {
               ...
               $where .= "       or  kode_shaff like '%".$search."%'";
               $where .= "       or  nama_shaff like '%".$search."%'";
        ...
        function findDataWhere($where){
            $sql = "SELECT * FROM master_shaff  ".$where;
            $sql .= " ORDER BY id";
            return $sql;
        }
        function findCountDataWhere($where){
            $sql = "SELECT count(id)  FROM master_shaff  ".$where;
            $sql .= " ORDER BY id";
            return $sql;
        }
        ...
        function createList($sql){
            $master_shaff_List = array();
            foreach ($this->dbh->query($sql) as $row) {
                    $master_shaff_ = new master_shaff();
                    $this->loadData($master_shaff_, $row);
                    $master_shaff_List[] = $master_shaff_;
            }
            return $master_shaff_List;            
        }

                
        function loadData($master_shaff,$row){
	    $master_shaff->setId($row['id']);
	    $master_shaff->setKode_shaff($row['kode_shaff']);
	    $master_shaff->setNama_shaff($row['nama_shaff']);
        ...
        function showAll(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $master_shaff_list = $this->showDataAllLimit();
                ...
                require_once './views/master_shaff/master_shaff_list.php';
            }else{
        ...
        function showAllJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $isexport = $this->isexport ;
                $isimport = $this->isimport;
                require_once './views/master_shaff/master_shaff_jquery_list.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showDetail(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $master_shaff_ = $this->showData($id);
                require_once './views/master_shaff/master_shaff_detail.php';
            }else{
                echo "You cannot access this module";
            }
        }
        function showDetailJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $master_shaff_ = $this->showData($id);
                require_once './views/master_shaff/master_shaff_jquery_detail.php';
            }else{
                echo  "You cannot access this module";
            }
        }
        ...
        function showForm(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $master_shaff_ = $this->showData($id);
                require_once './views/master_shaff/master_shaff_form.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showFormJQuery(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $search = isset($_REQUEST["search"]) ? $_REQUEST["search"] : "";
                $master_shaff_ = $this->showData($id);
                require_once './views/master_shaff/master_shaff_jquery_form.php';
            }else{
                echo "You cannot access this module";
            }
        }        
	    ...
	    $kode_shaff = isset($_POST['kode_shaff'])?$_POST['kode_shaff'] : "";
	    $nama_shaff = isset($_POST['nama_shaff'])?$_POST['nama_shaff'] : "";
	    $this->master_shaff->setId($id);
	    $this->master_shaff->setKode_shaff($kode_shaff);
	    $this->master_shaff->setNama_shaff($nama_shaff);            
            $this->saveData();
        }
        public function saveData(){
            $check = $this->checkData($this->master_shaff->getId());
            if($check == 0){
        ...
        public function getMaster_shaff() {
            return $this->master_shaff;
        }
        public function setMaster_shaff($master_shaff) {
            $this->master_shaff = $master_shaff;
        }
